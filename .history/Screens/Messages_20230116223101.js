...
const Messages = ({ navigation }) => {
  ...
  const searchRenderItem = ({ item }) => {
    return (
      <TouchableOpacity
        ...
        activeOpacity={1}
        onPress={() => goToConversation(item.id)}
      >
      ...
      >
        ...
        <View style={styles.details}>
          ...
          >
            ...
            <Text style={styles.talcsign}>{item.date}</Text>
          </View>
          ...
          >
            <Icon.Check
              style={{ fontSize: 14, color: "#A7A7A7", marginRight: 5 }}
              width={15}
              height={15}
              color="#F7941D"
            />
          <Text style={styles.talcsign} numberOfLines={2}>{item.message}</Text>
          </View> */}
        </View>
      </TouchableOpacity>
    );
  ...
  const goToConversation = async (userId) => {
    const ref = auth.currentUser.uid + userId || userId + auth.currentUser.uid;
    const chatRef = doc(firestore, "chats", auth.currentUser.uid + userId);
    try {
      ...
      if (chatDocument.exists()) {
        ...
        try {
          await setDoc(doc(firestore, "chats", auth.currentUser.uid + userId), {
            id: auth.currentUser.uid + userId,
