...
const CommentPage = ({ route, navigation }) => {
  ...
  const [commentsLikes, setCommentsLikes] = useState(false);
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity onPress={() => likeComment(item.id)}>
          {commentsLikes.includes(auth.currentUser.uid) ? (
  ...
  const likeComment = async (commentId) => {
    try {
      ...
      setCommentsLikes(commentSnapshot.data());
      if (commentsLikes.includes(auth.currentUser.uid)) {
